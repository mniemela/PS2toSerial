...
volatile static uint8_t mergePackets = 0;
...
static void handleStream(char c) {
	...
	if (bytesReceived == 2 && rxArray[0] == 0xAA && rxArray[1] == 0) {
		...
		if (!highSpeedUart && (mergePackets || (getUsedBuffer() > 0))) {
		...
		// it's possible that wheel movement data is lost if the packet had been sent to serial
		// after 3rd byte had been received but before 4th byte arrived. this can happen only
		// when merging packets as a KVM workaround
		if (skipped) {
